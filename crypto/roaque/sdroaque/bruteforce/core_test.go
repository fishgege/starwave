...
func decryptAndCheckHelper(t *testing.T, params *Params, key *PrivateKey, cipher CiphertextList, message *bn256.GT) {
...
func decryptAndCheckHelper2(t *testing.T, params *Params, key *PrivateKey, cipher CiphertextList, message *bn256.GT) {
	decrypted := Decrypt(params, key, cipher)
	if decrypted == nil || !bytes.Equal(message.Marshal(), decrypted.Marshal()) {
		return
	}
